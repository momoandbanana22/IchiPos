...
public class AppConfig
{
    ...
    /// <summary>
    /// 定型文(04書 G-016 第2節、issue #111)。GUIの「定型文」タブに、登録順のままワンクリック投稿ボタンとして並ぶ。
    /// 1件は Misskey本文・X本文を持つ(<see cref="TemplateEntry"/>)。設定では文字列(両者同一)または
    /// misskey/x のマップで書ける(<see cref="TemplateEntryYamlConverter"/>)。
