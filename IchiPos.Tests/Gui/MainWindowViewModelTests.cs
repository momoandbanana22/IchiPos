...
public class MainWindowViewModelTests
{
    ...
    // Misskey・X とも同じ本文の定型文（文字列登録相当）。
    private static TemplateEntry Tpl(string text) => new(text, text);

    private static AppConfig ConfigWithTemplates(params string[] templates)
    {
        var config = ValidConfig();
        config.Templates = templates.Select(Tpl).ToList();
        return config;
    }

    // Misskey本文とX本文が異なる定型文1件を持つ設定（issue #111）。
    private static AppConfig ConfigWithSplitTemplate(string misskey, string x)
    {
        var config = ValidConfig();
        config.Templates = new List<TemplateEntry> { new(misskey, x) };
        return config;
    }
    ...
    public void 初期状態_定型文一覧は設定の内容を登録順のまま公開する()
    {
        ...
        Assert.Equal(new[] { "おはよう", "おやすみ" }, vm.Templates.Select(t => t.MisskeyText));
    }

    [Fact]
    public void 初期状態_定型文が0件ならHasTemplatesがfalse()
    {
        // G-016第3節第5項: 一覧の代わりに未登録の案内を表示するためのView側の判定に使う。
    ...
    public async Task 正常系_定型文投稿は定型文テキストを投稿内容として渡す()
    {
        ...
        var mockApp = new Mock<IIchiPosApplication>();
        mockApp.Setup(x => x.RunAsync(It.IsAny<string>(), It.IsAny<IReadOnlyList<string>>(), config, It.IsAny<bool>(), It.IsAny<string>())).ReturnsAsync(0);
        var vm = BuildViewModel(app: mockApp, config: config);

        await vm.PostTemplateAsync(Tpl("おやすみ"));

        mockApp.Verify(x => x.RunAsync("おやすみ", It.IsAny<IReadOnlyList<string>>(), config, It.IsAny<bool>(), It.IsAny<string>()), Times.Once);
    }
    ...
    public async Task 正常系_定型文投稿は添付画像を渡さない()
    {
        ...
        await vm.PostTemplateAsync(Tpl("おはよう"));

        mockApp.Verify(x => x.RunAsync("おはよう", It.Is<IReadOnlyList<string>>(p => p.Count == 0), config, It.IsAny<bool>(), It.IsAny<string>()), Times.Once);
    }
    ...
    public async Task 正常系_定型文投稿中はIsBusyがtrueになる()
    {
        ...
        var config = ConfigWithTemplates("おはよう");
        var mockApp = new Mock<IIchiPosApplication>();
        mockApp.Setup(x => x.RunAsync(It.IsAny<string>(), It.IsAny<IReadOnlyList<string>>(), config, It.IsAny<bool>(), It.IsAny<string>()))
            .Returns(tcs.Task);
        var vm = BuildViewModel(app: mockApp, config: config);

        var postTask = vm.PostTemplateAsync(Tpl("おはよう"));
        Assert.True(vm.IsBusy);
        Assert.False(vm.IsNotBusy);
    ...
    public async Task 正常系_投稿中は定型文投稿コマンドを実行できない()
    {
        ...
        var config = ConfigWithTemplates("おはよう");
        var mockApp = new Mock<IIchiPosApplication>();
        // 通常の投稿(PostAsync)は4引数版を使うため、こちらを塞いで投稿中(IsBusy)を再現する。
        mockApp.Setup(x => x.RunAsync(It.IsAny<string>(), It.IsAny<IReadOnlyList<string>>(), config, It.IsAny<bool>()))
            .Returns(tcs.Task);
        var vm = BuildViewModel(app: mockApp, config: config);
        vm.Content = "hello";
        ...
        var postTask = vm.PostAsync();
        Assert.False(vm.PostTemplateCommand.CanExecute(Tpl("おはよう")));

        tcs.SetResult(0);
        await postTask;

        Assert.True(vm.PostTemplateCommand.CanExecute(Tpl("おはよう")));
    }
    ...
    public async Task 正常系_定型文投稿が失敗したら前回投稿内容を記録しない()
    {
        var config = ConfigWithTemplates("おはよう");
        var mockApp = new Mock<IIchiPosApplication>();
        mockApp.Setup(x => x.RunAsync(It.IsAny<string>(), It.IsAny<IReadOnlyList<string>>(), config, It.IsAny<bool>(), It.IsAny<string>())).ReturnsAsync(1);
        var store = new Mock<ILastPostStore>();
        var vm = BuildViewModel(app: mockApp, config: config, lastPostStore: store);
    ...
    public async Task 正常系_前回と同じ定型文なら再投稿確認を行う()
    {
        ...
        confirmation.Verify(x => x.ConfirmRepost(), Times.Once);
        mockApp.Verify(x => x.RunAsync("おはよう", It.IsAny<IReadOnlyList<string>>(), config, It.IsAny<bool>(), It.IsAny<string>()), Times.Once);
    }
    ...
    public async Task 正常系_定型文の再投稿確認でいいえを選ぶと投稿しない()
    {
        ...
        mockApp.Verify(x => x.RunAsync(It.IsAny<string>(), It.IsAny<IReadOnlyList<string>>(), It.IsAny<AppConfig>(), It.IsAny<bool>(), It.IsAny<string>()), Times.Never);
    ...
    public async Task 正常系_定型文の日付プレースホルダは投稿実行時に置換される()
    {
        ...
        await vm.PostTemplateAsync(Tpl("今日は{date}です"));

        // 置換は投稿処理層(F-013)が行うため、渡すのは未置換のテキストのまま。
        // 記録するハッシュは置換後のテキストで計算する(G-015第2節)。
        mockApp.Verify(x => x.RunAsync("今日は{date}です", It.IsAny<IReadOnlyList<string>>(), config, It.IsAny<bool>(), It.IsAny<string>()), Times.Once);
        store.Verify(x => x.SaveHash(HashOf("今日は2026/07/20です")), Times.Once);
    }

    // ──────────────────────────────────────────────────────────────────
    // 定型文: Misskey本文とX本文を分けて登録する(04書 G-016、issue #111)
    // ──────────────────────────────────────────────────────────────────

    [Fact]
    public void 初期状態_定型文一覧はMisskey本文とX本文を公開する_issue111()
    {
        var vm = BuildViewModel(config: ConfigWithSplitTemplate(":ohayou:", "おはよう"));

        var entry = Assert.Single(vm.Templates);
    ...
    [Fact]
    public async Task 正常系_定型文投稿はMisskey本文とX本文を投稿処理層へ渡す_issue111()
    {
        // issue #111: Misskeyには:ohayou:を、Xにはおはようを渡す。
        var config = ConfigWithSplitTemplate(":ohayou:", "おはよう");
        var mockApp = new Mock<IIchiPosApplication>();
        mockApp.Setup(x => x.RunAsync(It.IsAny<string>(), It.IsAny<IReadOnlyList<string>>(), config, It.IsAny<bool>(), It.IsAny<string>())).ReturnsAsync(0);
        var vm = BuildViewModel(app: mockApp, config: config);

        await vm.PostTemplateAsync(new TemplateEntry(":ohayou:", "おはよう"));

        // content=Misskey本文, xContent=X本文 として5引数版へ渡す。
        mockApp.Verify(x => x.RunAsync(":ohayou:", It.Is<IReadOnlyList<string>>(p => p.Count == 0), config, It.IsAny<bool>(), "おはよう"), Times.Once);
    }

    [Fact]
    public async Task 正常系_定型文の再投稿確認と記録はMisskey本文で行う_issue111()
    {
        // G-016第5節第2項: 比較・記録に用いるのはMisskey本文とする。
        var config = ConfigWithSplitTemplate(":ohayou:", "おはよう");
        var mockApp = new Mock<IIchiPosApplication>();
        mockApp.Setup(x => x.RunAsync(It.IsAny<string>(), It.IsAny<IReadOnlyList<string>>(), config, It.IsAny<bool>(), It.IsAny<string>())).ReturnsAsync(0);
        var store = new Mock<ILastPostStore>();
        // 前回の記録がMisskey本文のハッシュと一致するので確認が出る。
        store.Setup(x => x.LoadHash()).Returns(HashOf(":ohayou:"));
        var confirmation = new Mock<IRepostConfirmation>();
        confirmation.Setup(x => x.ConfirmRepost()).Returns(true);
        var vm = BuildViewModel(app: mockApp, config: config, lastPostStore: store, repostConfirmation: confirmation);
        ...
        confirmation.Verify(x => x.ConfirmRepost(), Times.Once);
        store.Verify(x => x.SaveHash(HashOf(":ohayou:")), Times.Once);
    }
    ...
    public void 異常系_再読み込み失敗時_設定を維持する()
    {
        ...
        Assert.Equal(new[] { "おはよう" }, vm.Templates.Select(t => t.MisskeyText));
        Assert.Equal("5 / 280 文字", vm.CharacterCountDisplay);
    }
